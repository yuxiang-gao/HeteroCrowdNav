...
class EnvPlayer(object):
    ...
    def key_release(self, k, mod):  # reverse action of pressed
        ...
        #     self.action[1] = 0
        if k in [key.RIGHT, key.D] and self.action[1] == -ROT_SPEED:
            self.action[1] = 0
        if k in [key.LEFT, key.A] and self.action[1] == +ROT_SPEED:
            self.action[1] = 0
        if k in [key.UP, key.W] and self.action[0] == +FRONT_SPEED:
            self.action[0] = 0
        if k in [key.DOWN, key.S] and self.action[0] == -BACK_SPEED:
    ...
    def reset(self):
        ...
        self.env.reset()
        # reset player
        self.realtime_rate = 1.0 / self.env.get_time_step()
